...
"""
"""Alice: This file is modified so that I can generate dataset manually
   (meaning I will manually move the robot arm to designated position)
   It is also modified to perform eye to hand calibration only, but I 
   think you only need slight modification to perform eye in hand, good luck
"""
import argparse
import datetime
import time
...
def _get_frame_and_transform_matrix(
    ...
    robot_pose = np.array(con.get_tcp_pos())
...
def _verify_good_capture(frame: zivid.Frame) -> None:
    ...
    if not detection_result.valid():
        print("Failed to detect feature points from captured frame.")
        return detection_result.valid()
    else:
        if input("Do you want to save this image?(y/other keys)") == 'y':
            return True
...
def _capture_one_frame_and_robot_pose(
    ...
    if _verify_good_capture(frame):
        ...
        print("since the image captured is not perfect, image is not saved")
        return False


def _generate_dataset(app: zivid.Application, con: robotarmcontrol.URRobot) -> Path:
    ...
    with app.connect_camera() as camera:
        ...
        while input("press q to end") != 'q':
            ...
            if _capture_one_frame_and_robot_pose(
                con,
                camera,
                settings,
                save_dir,
...
def perform_hand_eye_calibration(
    ...
    while True:
        ...
        print(frame_file_path.is_file())
        print(pose_file_path.is_file())
        if frame_file_path.is_file() and pose_file_path.is_file():
            ...
            detection_result = zivid.calibration.detect_feature_points(point_cloud)
            if not detection_result.valid():
                print(f"Failed to detect feature points from frame {frame_file_path}")
            else:
...
def _main() -> None:
    ...
    robot = robotarmcontrol.URRobot(ROBOT_IP)
    #robot_ip_address = user_options.ip
    #input_data = _initialize_robot_sync(robot_ip_address)
    #con.send_start()

    dataset_dir = _generate_dataset(app, robot)
